...
> [!TIP]
> Return any non-undefined value (see [Response Types](/guide/event-handler#response-types)) to produce a [`200 OK`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/200) status response.
...
You can define a custom matcher function to have more advanced logic for matching requests, but simpler than a [router](/guide/router).
...
You can provide an async function that h3 will load the first time a request matching the route is received. It can be useful for dynamic imports to reduce startup time.
...
> [!IMPORTANT]
> _These details are mainly informational and are used for `h3` development._
> 
> _**never** use `h3` internals directly for production applications!_

The `h3` app instance has the following additional internal properties available:
...
- `app.handler`: Direct stack handler function (**unsafe to call directly**).
